...
# ========= RULES IMPLEMENTATION AND BOARD MANAGEMENT ======
...
def legal_moves_rdm(game, color):
    lm = []
    for move in pseudo_legal_moves(game, color):
        if is_legal_move(game, move):
            lm = lm + [move]
...
def get_AI_move(game, level, depth=2, use_book=False):
    if verbose:
...
def watch_AI_game(level, game=Game(), sleep_seconds=0.5):
    ...
    while True:
        ...
        game = make_move(game, get_AI_move(game, level))
        sleep(sleep_seconds)
    print_outcome(game)
